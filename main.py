        ...
        for j in range((self.rect.y - game.padding) // WIDTH - self.scale, self.scale + 1 + (self.rect.y - game.padding) // WIDTH):
            for i in range((self.rect.x - game.padding) // WIDTH - self.scale, (self.rect.x - game.padding) // WIDTH + self.scale + 1):
                ...
                try:
                    ...
                    print("ITS THAT BUG!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!", i, j, self.rect.x // WIDTH, self.rect.y // WIDTH)
                    print(len(self.maze.layout), len(self.maze.layout[0]))
                    print(self.path)
...
class GameState:
    def __init__(self, m, n, WDITH, PADDING, SPARSITY, Fill_color):
        ...
        self.entities.append(Gift(self.screen, self.maze, (WIDTH + PADDING, PADDING + WIDTH), (r.randint(0, 255),r.randint(0, 255),r.randint(0, 255)), 90, 1, 2, 0))
        ...
        self.inventory = [99, 5, 5, 5] # bullets,powered,mines,trap-clear
    ...
    def start_level(self):
        ...
        while self.lives > 0:
            ...
            self.entities = [Builder(self.screen, self.maze, (self.padding + self.m * WIDTH // 2, self.padding + self.n * WIDTH // 2), (0, 255, 0), 80, 1, 2, .15) for i in range(12)]
            # self.entities = [Entity(self.screen, self.maze, (self.padding + self.m * WIDTH // 2, self.padding + self.n * WIDTH // 2), (0, 255, 0), 80, 1, 2, 0) for i in range(self.level * 2)]
            ...
            for i in range(2 * self.level):
                self.entities.append(Stalker(self.screen, self.maze, (self.padding + self.m * WIDTH // 2, self.padding + self.n * WIDTH // 2), (0, 255, 0), 30, 1, 2, 2))
        
            # self.entities += [Snake(self.screen, self.maze, (0, 255, 0), (self.padding + self.m * WIDTH // 2, self.padding + self.n * WIDTH // 2), "unravel", 1, self.level * 3, 8) for i in range(self.level)]
            self.traps = []
    ...
    def run(self):
            ...
            for e in pygame.event.get():
                ...
                if e.type == pygame.KEYDOWN:
                    ...
                    if e.key == pygame.K_RSHIFT:
                        return 1
                    if e.key == pygame.K_0:
                        self.entities.append(Gift(self.screen, self.maze, (WIDTH + self.padding, self.padding + WIDTH), (255, 255, 0), 75, 1, 4, 0))
                    if e.key == pygame.K_9:
                        self.player.incHits()
        ...
        # Move the player if an arrow key is pressed
            ...
            if len(self.entities) > 0:
                if self.entities[-1].color != (255, 255, 0) and self.player.hits > 5:
                    self.entities.append(Gift(self.screen, self.maze, (WIDTH + self.padding, self.padding + WIDTH), (255, 255, 0), 80, 1, 4, 0))
                    print("GIFT!")
