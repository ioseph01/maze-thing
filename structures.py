...
class Breakable_Wall(Wall):
    ...
    def doSomething(self):
        ...
        self.render()
        if self.rect.colliderect(self.game.player.rect) and self.game.player.state == "alive":
            self.die()
...
class Maze(object):
    ...
    def __init__(self, game, row, col, sparsity, padding, WALL_COLOR):
        ...
        self.layout[1][1][0].color = (255, 255, 255)
        ...
        for row in range(len(self.layout)):
            ...
            for col in range(len(self.layout[row])):
                if isinstance(self.layout[row][col][0], Breakable_Wall):
                    ...
                    copy[row].append([Explosive_Wall(self.game, self.layout[row][col][0].pos, (208, 4, 8))])
    ...
    def combine(self, layout, other, sparsity):
        ...
        for y in range(1, len(layout) - 1):
            for x in range(1, len(layout[y]) - 1):
                ...
                if  r_int(0, 100 ) < sparsity[1] and x != 0 and x != len(self.layout[0]) - 1 and y != 0 and y != len(self.layout) - 1 and x % 2 != 1 and y % 2 != 1:
                    layout[y][x][0].die()
                elif r_int(0, explosive_) < 10 and x % 2 != 1 and y % 2 != 1:
                    layout[y][x][0] = Explosive_Wall(self.game, self.layout[y][x][0].pos, (208, 4, 8))
    ...
    def change_color(self):
        ...
        glass = (color[0] // 2, color[1] // 2, color[2] // 2)
        self.game.wall_color = color
     
        for row in self.layout:
            for wall in row:
                if wall[0].state != "dead":
                    if isinstance(wall[0], Explosive_Wall):
                        continue
                    elif isinstance(wall[0], Breakable_Wall):
                        wall[0].color = glass
                    else:
                        wall[0].color = color
                if len(wall) == 2:
