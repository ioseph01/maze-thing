...
class Entity(Unit):
    ...
    def move_path(self):
        ...
        try:
            ...
            self.trace_tick = (self.trace_tick + 1) % ((WIDTH * self.scale) // self.step)
...
class Explosion(Unit):
    ...
    def __init__(self, game, color, coords, scale, hp):
        ...
        super(Explosion, self).__init__(game, color, (coords[0] - WIDTH * a, coords[1] - WIDTH * a), "powered", "north", scale, 4, hp)
        self.death_sound = "boom.mp3"
...
class Gift(Entity):
    ...
    def reward(self):
        for i in range(len(self.game.inventory)):
            if self.game.inventory[i] < 10:
                self.game.inventory[i] = self.game.inventory[i] + 2 + self.game.level // 10 if self.game.inventory[i] + 2 + self.game.level // 10 < 99 else 99
            elif self.game.inventory[i] < 20:
                self.game.inventory[i] = self.game.inventory[i] + 1 + self.game.level // 10 if self.game.inventory[i] + 1 + self.game.level // 10 < 99 else 99
            else:
                self.game.inventory[i] = self.game.inventory[i] + self.game.level // 10 if self.game.inventory[i] + self.game.level // 10 < 99 else 99
